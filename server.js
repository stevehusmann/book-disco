...
const seedPath = path.join(__dirname, 'public', 'BookList.json');
...
app.post('/api/books', (req, res) => {
  const nextBook = {
    ...req.body,
    _uid: req.body?._uid || req.body?.uid || req.body?.ISBN || req.body?.EAN || `book-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`
  };
  const params = toDbParams(nextBook);
